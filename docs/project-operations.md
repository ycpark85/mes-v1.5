...
- Canceling an outsource work group cancels its pending inspection schedule but does not cancel the production LOT. When no other active outsource work or inspection remains, a `WAITING` or `RECEIVED` LOT returns to stored status `WAITING` and is displayed as `CREATED` so it can be instructed again.
